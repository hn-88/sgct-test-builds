...
void networkAck(int packageId, int) {
    Log::Info(sgct::format("Network package {} is received", packageId));
...
void networkDecode(void* receivedData, int receivedLength, int packageId, int) {
    Log::Info(sgct::format("Network decoding package {}", packageId));
    std::string test(reinterpret_cast<char*>(receivedData), receivedLength);
    Log::Info(sgct::format("Message: \"{}\"", test));
}
...
void connect() {
    ...
    // init
    try {
        Log::Debug(sgct::format("Initiating network connection at port {}", port));
        ...
        Log::Error(sgct::format("Network error: {}", err.what()));
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (v == "-port" && argc > (i + 1)) {
            ...
            Log::Info(sgct::format("Setting port to: {}", port));
        }
        else if (v == "-address" && argc > (i + 1)) {
            address = argv[i + 1];
            Log::Info(sgct::format("Setting address to: {}", address));
        }
        else if (v == "--server") {
            isServer = true;
