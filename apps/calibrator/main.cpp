...
void initializeBox() {
    ...
    if (!std::filesystem::exists(front.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", front.filename));
        exit(EXIT_FAILURE);
    }
    ...
    if (!std::filesystem::exists(right.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", right.filename));
        exit(EXIT_FAILURE);
    }
    ...
    if (!std::filesystem::exists(back.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", back.filename));
        exit(EXIT_FAILURE);
    }
    ...
    if (!std::filesystem::exists(left.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", left.filename));
        exit(EXIT_FAILURE);
    }
    ...
    if (!std::filesystem::exists(top.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", top.filename));
        exit(EXIT_FAILURE);
    }
    ...
    if (!std::filesystem::exists(bottom.filename)) {
        Log::Error(sgct::format("Could not find image '{}'", bottom.filename));
        exit(EXIT_FAILURE);
    }
...
void postDraw() {
    if (runTests) {
        frameNumber++;
        Log::Info(sgct::format("Frame: {}", frameNumber));
    }
