...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (arg == "-tex" && argc > (i + 1)) {
            ...
            numberOfTextures++;
            Log::Info(sgct::format("Adding texture: {}", argv[i + 1]));
        }
            ...
            Log::Info(sgct::format("Start frame set to {}", startFrame));
        }
            ...
            Log::Info(sgct::format("Format set to {}", argv[i + 1]));
        }
        else if (arg == "-path" && argc > (i + 1)) {
            Settings::instance().setCapturePath(argv[i + 1]);
            Log::Info(sgct::format("Left path set to {}", argv[i + 1]));
        }
