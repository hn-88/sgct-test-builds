...
void drawOmniStereo(const RenderData& renderData) {
    ...
    Log::Info(sgct::format("Time to draw frame: {}s", time() - t0));
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (argument == "-turnmap" && argc > i + 1) {
            turnMapSrc = argv[i + 1];
            Log::Info(sgct::format("Setting turn map path to {}", turnMapSrc));
        }
        if (argument == "-sepmap" && argc > i + 1) {
            sepMapSrc = argv[i + 1];
            Log::Info(sgct::format("Setting separation map path to '{}'", sepMapSrc));
        }
    }
