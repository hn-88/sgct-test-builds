...
TEST_CASE("Load: CubemapProjection/Channels", "[parse]") {
    constexpr auto idxToChannels = [](uint8_t idx) {
        ...
        res2.push_back(sgct::format("\"right\": {}", idx & 0b000001 ? "true" : "false"));
        res1.zLeft = idx & 0b000010;
        res2.push_back(sgct::format("\"zleft\": {}", idx & 0b000010 ? "true" : "false"));
        res1.bottom = idx & 0b000100;
        res2.push_back(sgct::format("\"bottom\": {}", idx & 0b000100 ? "true" : "false"));
        res1.top = idx & 0b001000;
        res2.push_back(sgct::format("\"top\": {}", idx & 0b001000 ? "true" : "false"));
        res1.left = idx & 0b010000;
        res2.push_back(sgct::format("\"left\": {}", idx & 0b010000 ? "true" : "false"));
        res1.zRight = idx & 0b100000;
        res2.push_back(sgct::format("\"zright\": {}", idx & 0b100000 ? "true" : "false"));
    ...
    for (uint8_t i = 0; i < 0b111111; i++) {
        ...
        std::string String = sgct::format(R"(
{{
  "version": 1,
  "masteraddress": "localhost",
  "nodes": [
