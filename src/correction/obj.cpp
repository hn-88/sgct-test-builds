...
Buffer generateOBJMesh(const std::filesystem::path& path) {
    ...
    Log::Info(sgct::format("Reading Wavefront OBJ mesh data from '{}'", path));

    std::ifstream file = std::ifstream(path);
    if (!file.good()) {
        throw Error(
            Error::Component::OBJ, 2030, sgct::format("Failed to open '{}'", path)
        );
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (first == "v") {
            ...
            if (std::find(reported.begin(), reported.end(), "vn") == reported.end()) {
                Log::Warning(sgct::format("Ignoring normals in mesh '{}'", path));
            ...
            if (std::find(reported.begin(), reported.end(), "vp") == reported.end()) {
                Log::Warning(
                    sgct::format("Ignoring parameter space values in mesh '{}'", path)
                );
            ...
            if (std::find(reported.begin(), reported.end(), "l") == reported.end()) {
                Log::Warning(sgct::format("Ignoring line elements in mesh '{}'", path));
                reported.emplace_back("l");
            }
        }
        else if (first == "mtllib") {
            if (std::find(reported.begin(), reported.end(), "mtllib") == reported.end()) {
                Log::Warning(sgct::format("Ignoring material library in mesh '{}'", path));
            ...
            if (std::find(reported.begin(), reported.end(), "usemtl") == reported.end()) {
                Log::Warning(
                    sgct::format("Ignoring material specification in mesh '{}'", path)
                );
            ...
            if (std::find(reported.begin(), reported.end(), "o") == reported.end()) {
                Log::Warning(
                    sgct::format("Ignoring object specification in mesh '{}'", path)
                );
            ...
            if (std::find(reported.begin(), reported.end(), "g") == reported.end()) {
                Log::Warning(
                    sgct::format("Ignoring object group specification in mesh '{}'", path)
                );
            ...
            if (std::find(reported.begin(), reported.end(), "s") == reported.end()) {
                Log::Warning(
                    sgct::format("Ignoring shading specification in mesh '{}'", path)
                );
