                                     ...
                                     std::string_view fragmentSrc)
{
    ...
    if (shaderProgramExists(name)) {
        throw Error(
            7000,
            sgct::format("Cannot add shader program '{}': Already exists", name)
        );
    }
...
bool ShaderManager::removeShaderProgram(std::string_view name) {
    ...
    if (shaderIt == _shaderPrograms.end()) {
        Log::Warning(
            sgct::format("Unable to remove shader program '{}': Not found", name)
        );
        return false;
    }
...
const ShaderProgram& ShaderManager::shaderProgram(std::string_view name) const {
    ...
    if (shaderIt == _shaderPrograms.end()) {
        throw Error(7001, sgct::format("Could not find shader with name '{}'", name));
    }
    return *shaderIt;
}
